    ...
    <b-row v-show="isAllLoaded()">
      <b-col class="m-0 p-0" md="8">
        <b-col cols="12">
          ...
          >
            <video
              :poster="`${apiUrl}films/${filmId}/thumbnail/poster`"
        ...
        <b-col class="pl-3 pr-3 mt-4" sm="12" v-if="film">
          <b-row class="m-0 p-0">
            <b-col class="text-justify p-0" sm="12">
              <p class="font-weight-bold">{{ this.film.title }}</p>
            </b-col>
            <b-col class="text-justify p-0" cols="4" sm="4">
            ...
            <b-col
              class="mt-4 mb-4 dropdown-divider ml-auto mr-auto"
              cols="12"
              sm="12"
            />
        ...
        <b-col class="p-0 mt-4 d-none d-md-block">
          <Comments
            ...
            v-bind:film-id="filmId"
            v-bind:is-logged-in="isLoggedIn"
          />
        </b-col>
      </b-col>
      <b-col class="p-0" cols="12" md="4" style="margin-left: auto">
        <Playlist
          v-bind:film-id="filmId"
          v-bind:film-video-height="this.filmVideoHeight"
...
export default {
  ...
  methods: {
    ...
    isAllLoaded() {
      const allLoaded =
        ...
        !this.content.filmsLoading &&
        !this.content.commentsLoading;
      if (allLoaded) {
    ...
    async loadFilm(id) {
      await service
        .updateFilmMetaViews(id, { viewed: true })
        .then((response) => {
    ...
    async handleLike(like) {
      await service
        .updateFilmMetaLikes(this.film.id, { liked: like })
        .then((response) => {
          this.film = response.data;
          if (like === "liked") {
  ...
  async created() {
    ...
    await service
      .updateFilmMetaViews(this.$route.params.id, { viewed: true })
      .then((response) => {
        this.film = response.data;
        this.checkUserLikes();
      })
