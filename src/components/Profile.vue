<template>
  <b-col class="m-0">
    ...
    <b-row v-if="isLoading === false">
      ...
      >
        ...
        >
          ...
          >
            <img
              alt=""
              :src="`${apiUrl}films/${film.id}/thumbnail/preview`"
              class="embed-responsive-item image"
            />
          ...
          <b-row class="p-0 m-0 mt-1">
            <b-col class="p-0 m-0 text-justify pr-2" cols="8" sm="8">
              <b-col class="p-0 mb-2">
                <span class="mb-1 mt-1 title" v-line-clamp:20="2">{{
                  film.title
                }}</span>
              </b-col>
      ...
      >
        <b-col class="m-0 p-0 playlist-remove-container m-container">
          <div
            class="
          ...
          >
            <PlaylistThumbnail
              @loaded="onPlaylistThumbnailLoaded($event, index)"
      ...
      .then((response) => {
        ...
        this.playlists.forEach((_playlist) => {
          ...
          _playlist.films.forEach((_film) => {
            _playlist.thumbnails.push(
              `${this.apiUrl}films/${_film}/thumbnail/preview`
            );
          });
  ...
  async mounted() {
    ...
    EventBus.$on("playlist_changed_profile", (playlist) => {
      ...
      playlist.films.forEach((_film) => {
        playlist.thumbnails.push(
          `${this.apiUrl}films/${_film}/thumbnail/preview`
        );
      });
      playlist.thumbnails.push(this.notFoundFilmsThumbnail);
      playlist.thumbnails.forEach(
        (_thumbnail) => _thumbnail + "?" + Date.now()
      );
      playlist.thumbnail = playlist.thumbnails[0];
      this.playlists[this.playlists.findIndex((el) => el.id === playlist.id)] =
        playlist;
      this.playlists = this.playlists.filter(
        (_playlist) => _playlist.id !== null
      );
    });
  },
  methods: {
